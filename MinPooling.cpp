 ...
 * MinPooling.c
 *
 * Implements the min-pooling transfer function. Takes a 4D tensor shaped as
 * (rows, cols, nchannels, nsamples) and a pooling shape as (prows, pcols) and
 * returns a set of min-values with the corresponding indices in the input
 * matrix.
 *
 * e.g.
 *      [m, idx] = MinPooling(IM, [2 2])
 *
...
/**
 * Computes the min-pooling for the given 2D map, and no, the name is not a typo.
 ...
 * @param ptr_out pointer to the output min-values set to the right position
 ...
 */
template <typename T>
void mexMinPooling(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[], mxClassID classID)
{
  ...
  /***************************************************************************/
  /* OUTPUTS: min-values and corresponding indices */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (nlhs !=2)
    mexErrMsgTxt("Must have 2 output arguments ([min_value, idxs])");
  ...
  if (classID == mxSINGLE_CLASS) {
      ...
      mexMinPooling<float>(nlhs, plhs, nrhs, prhs, classID);
  }  else if (classID == mxDOUBLE_CLASS) {
      if (debug)
        fprintf(stderr, "Executing the double version\n");

      mexMinPooling<double>(nlhs, plhs, nrhs, prhs, classID);
  }
}
